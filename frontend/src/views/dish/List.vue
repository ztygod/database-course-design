...
export default class DishList extends Vue {
  ...
  // 配料相关
  private ingredientDialogVisible = false;
  private currentDish: Partial<Dish> = {};
  private dishIngredients: DishIngredient[] = [];
  private ingredientOptions: Ingredient[] = [];
  private ingredientForm = {
  ...
  private handleEdit(row: Dish) {
    this.dialogTitle = '编辑菜品';
    this.form = { ...row };
    this.dialogVisible = true;
  }
  
  private async handleDelete(row: Dish) {
    try {
  ...
  private async handleStatusChange(row: Dish) {
    try {
  ...
  private async handleRemoveIngredient(row: DishIngredient) {
    try {
